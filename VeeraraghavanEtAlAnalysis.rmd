...
**McNemar's test to assess the difference between the classifiers**

```{r, echo=FALSE, warning=FALSE}
 
  accVals <- data.frame(c(rep("GT", length(rfGTHER2)+length(rfGTERPRTN)), 
                          rep("GC", length(rfGTHER2)+length(rfGTERPRTN)), 
                          rep("GCGMM", length(rfGTHER2)+length(rfGTERPRTN)), 
                          rep("FCM", length(rfGTHER2)+length(rfGTERPRTN))), 
                          c(rep("HER2+vs.notHER2+", length(rfGTHER2)), 
                            rep("ERPR+vs.TN", length(rfGTERPRTN)),  
                           rep("HER2+vs.notHER2+", length(rfGTHER2)), 
                           rep("ERPR+vs.TN", length(rfGTERPRTN)), 
                          rep("HER2+vs.notHER2+", length(rfGTHER2)), 
                          rep("ERPR+vs.TN", length(rfGTERPRTN)), 
                          rep("HER2+vs.notHER2+", length(rfGTHER2)), 
                          rep("ERPR+vs.TN", length(rfGTERPRTN))), 
                        c(rfGTHER2, rfGTERPRTN, rfGCHER2, rfGCERPRTN, 
                          rfGCGMMHER2, rfGCGMMERPRTN,  
                          rfFCMHER2, rfFCMERPRTN))
  colnames(accVals) <- c("Method", "Classifier", "Response")
  
  testRes <- data.frame(c(rep("HER2+vs.notHER2+",3), rep("ERPR+vs.TN",3)), 
                        c("GT:GC", "GT:GCGMM", "GT:FCM", "GT:GC", "GT:GCGMM", 
                          "GT:FCM"), 
                        c(rep(0,6)), c(rep(0,6)))
  colnames(testRes) <- c("Classifier", "Methods", "ChiSq", "P-value")
                                        
  v1 = accVals$Response[accVals$Method=="GT" & accVals$Classifier=="HER2+vs.notHER2+"]
  v2 = accVals$Response[accVals$Method=="GC" & accVals$Classifier=="HER2+vs.notHER2+"]
  performance <- matrix(c(sum(v1==1), sum(v1==2), sum(v2==1), sum(v2==2)), nrow=2, dimnames = list("M1" = c("HER2+", "HER2-"), "M2" = c("HER2+", "HER2-")))
  
  res <- mcnemar.test(performance)
    
  testRes[1,3] = res$statistic
  testRes[1,4] = res$p.value
  
  v1 = accVals$Response[accVals$Method=="GT" & accVals$Classifier=="ERPR+vs.TN"]
  v2 = accVals$Response[accVals$Method=="GC" & accVals$Classifier=="ERPR+vs.TN"]
  performance <- matrix(c(sum(v1==1), sum(v1==2), sum(v2==1), sum(v2==2)), nrow=2, dimnames = list("M1" = c("HER2+", "HER2-"), "M2" = c("HER2+", "HER2-")))
  
  res <- mcnemar.test(performance)
  
  testRes[4,3] = res$statistic
  testRes[4,4] = res$p.value
  
  
  v1 = accVals$Response[accVals$Method=="GT" & accVals$Classifier=="HER2+vs.notHER2+"]
  v2 = accVals$Response[accVals$Method=="GCGMM" & accVals$Classifier=="HER2+vs.notHER2+"]
  performance <- matrix(c(sum(v1==1), sum(v1==2), sum(v2==1), sum(v2==2)), nrow=2, dimnames = list("M1" = c("HER2+", "HER2-"), "M2" = c("HER2+", "HER2-")))
  
  res <- mcnemar.test(performance)
    
  testRes[2,3] = res$statistic
  testRes[2,4] = res$p.value
  
  v1 = accVals$Response[accVals$Method=="GT" & accVals$Classifier=="ERPR+vs.TN"]
  v2 = accVals$Response[accVals$Method=="GCGMM" & accVals$Classifier=="ERPR+vs.TN"]
  performance <- matrix(c(sum(v1==1), sum(v1==2), sum(v2==1), sum(v2==2)), nrow=2, dimnames = list("M1" = c("HER2+", "HER2-"), "M2" = c("HER2+", "HER2-")))
  
  res <- mcnemar.test(performance)
  
  
  testRes[5,3] = res$statistic
  testRes[5,4] = res$p.value
  
  
  v1 = accVals$Response[accVals$Method=="GT" & accVals$Classifier=="HER2+vs.notHER2+"]
  v2 = accVals$Response[accVals$Method=="FCM" & accVals$Classifier=="HER2+vs.notHER2+"]
  performance <- matrix(c(sum(v1==1), sum(v1==2), sum(v2==1), sum(v2==2)), nrow=2, dimnames = list("M1" = c("HER2+", "HER2-"), "M2" = c("HER2+", "HER2-")))
  
  res <- mcnemar.test(performance)
  
  testRes[3,3] = res$statistic
  testRes[3,4] = res$p.value
  
  
  v1 = accVals$Response[accVals$Method=="GT" & accVals$Classifier=="ERPR+vs.TN"]
  v2 = accVals$Response[accVals$Method=="FCM" & accVals$Classifier=="ERPR+vs.TN"]
  performance <- matrix(c(sum(v1==1), sum(v1==2), sum(v2==1), sum(v2==2)), nrow=2, dimnames = list("M1" = c("HER2+", "HER2-"), "M2" = c("HER2+", "HER2-")))
  
  res <- mcnemar.test(performance)
  
  testRes[6,3] = res$statistic
  testRes[6,4] = res$p.value
  
  print("McNemar Tests")
  kable(testRes, digits = 2, aling="l")
  
```

**Were there differences in accuracies between GCGMM and other algorithms?**

```{r, echo=FALSE, warning=FALSE}

  
  accVals <- data.frame(c(rep("GC", length(rfGTHER2)+length(rfGTERPRTN)), 
                          rep("GCGMM", length(rfGTHER2)+length(rfGTERPRTN)), 
                          rep("FCM", length(rfGTHER2)+length(rfGTERPRTN))), 
                          c(rep("HER2+vs.notHER2+", length(rfGTHER2)), 
                           rep("ERPR+vs.TN", length(rfGTERPRTN)), 
                          ...
                          rep("HER2+vs.notHER2+", length(rfGTHER2)), 
                          rep("ERPR+vs.TN", length(rfGTERPRTN))), 
                        c(rfGCHER2, rfGCERPRTN, 
                          rfGCGMMHER2, rfGCGMMERPRTN,  
                          rfFCMHER2, rfFCMERPRTN))
  colnames(accVals) <- c("Method", "Classifier", "Response")
  
  testRes <- data.frame(c(rep("HER2+vs.notHER2+",2), rep("ERPR+vs.TN",2)), 
                        c("GCMM:GC", "GCGMM:FCM", "GCGMM:GC", "GCGMM:FCM"), 
                        c(rep(0,4)), c(rep(0,4)))
  colnames(testRes) <- c("Classifier", "Methods", "ChiSq", "P-value")
                                        
  v1 = accVals$Response[accVals$Method=="GCGMM" & accVals$Classifier=="HER2+vs.notHER2+"]
  v2 = accVals$Response[accVals$Method=="GC" & accVals$Classifier=="HER2+vs.notHER2+"]
  performance <- matrix(c(sum(v1==1), sum(v1==2), sum(v2==1), sum(v2==2)), nrow=2, dimnames = list("M1" = c("HER2+", "HER2-"), "M2" = c("HER2+", "HER2-")))
  ...
  v1 = accVals$Response[accVals$Method=="GCGMM" & accVals$Classifier=="ERPR+vs.TN"]
  v2 = accVals$Response[accVals$Method=="GC" & accVals$Classifier=="ERPR+vs.TN"]
  performance <- matrix(c(sum(v1==1), sum(v1==2), sum(v2==1), sum(v2==2)), nrow=2, dimnames = list("M1" = c("HER2+", "HER2-"), "M2" = c("HER2+", "HER2-")))
